...
public class UsuariosDao {
   ...
   try{
    ...
    public static int getContagem() {
        ...
        try{
            Connection con = getConnection();
            PreparedStatement ps = (PreparedStatement) con.prepareStatement("SELECT count(*) AS ativo FROM usuarios where status = 'ativo'");
            ResultSet rs = ps.executeQuery();
            while(rs.next()){
                valores[0] = rs.getInt("Ativo");
            }   
 
            ps = (PreparedStatement) con.prepareStatement("SELECT count(*) AS inativo FROM usuarios where status = 'inativo'");
            rs = ps.executeQuery();
            while(rs.next()){
                valores[1] = rs.getInt("Inativo");
            }    
            
            ps = (PreparedStatement) con.prepareStatement("SELECT count(*) AS aguardando FROM usuarios where status = 'aguardando'");
            rs = ps.executeQuery();
            while(rs.next()){
